...
function addBookToLibrary() {
  ...
  const newBook = new Book(bookForm.elements['title'].value, 
...
dbLibraryRef.on('value', snap => {
  ...
  function changeRead(event) {
    dbLibraryRef.child(`${event.target.parentNode.parentNode.id}/read`).once('value').then(snapshot => {
      if(snapshot.val() === 'unread') {
        dbLibraryRef.child(`${event.target.parentNode.parentNode.id}`).update({read: 'read'})
      } else {
        dbLibraryRef.child(`${event.target.parentNode.parentNode.id}`).update({read: 'unread'})
      }
  })
}
function openForm() {
